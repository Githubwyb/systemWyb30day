...
// static inline void taskswitch4(void) {
//     // 计算目标地址
//     unsigned int segment = 5 * 8;
//     unsigned int offset = 0;
//     unsigned int target_address = (segment << 4) + offset;

//     // 内联汇编实现JMP指令
//     __asm__ __volatile__("jmp *%0" : : "r"(target_address));
// }
...
void HariMain(void) {
    ...
    // tss_a.x86_tss.ldt = 0;
    // tss_a.x86_tss.io_bitmap_base = 0x4000;
    // tss_b.x86_tss.ldt = 0;
    // tss_b.x86_tss.io_bitmap_base = 0x4000;

    // tss_b.x86_tss.ip = (int)&task_b_main;
    ...
    // tss_b.x86_tss.si = 0;
    // tss_b.x86_tss.di = 0;
    // tss_b.x86_tss.es = 1 * 8;
    // tss_b.x86_tss.cs = 2 * 8;
    // tss_b.x86_tss.ss = 1 * 8;
    // tss_b.x86_tss.ds = 1 * 8;
    // tss_b.x86_tss.fs = 1 * 8;
    // tss_b.x86_tss.gs = 1 * 8;

    // 初始化GDT
    // struct desc_struct *gdt = (struct desc_struct *)ADR_GDT;
    // tss_desc tss_a_desc;
    // tss_desc tss_b_desc;
    // set_tssldt_descriptor(&tss_a_desc, (unsigned long)&tss_a.x86_tss, DESC_TSS,
    // 		      __KERNEL_TSS_LIMIT);
    // set_tssldt_descriptor(&tss_b_desc, (unsigned long)&tss_b.x86_tss, DESC_TSS,
    // 		      __KERNEL_TSS_LIMIT);
    // write_gdt_entry(gdt, 4, &tss_a_desc, DESC_TSS);
    // write_gdt_entry(gdt, 5, &tss_b_desc, DESC_TSS);
    // load_tr_desc(4 * 8);
    tss_a.ldtr = 0;
    tss_a.iomap = 0x40000000;
    tss_b.ldtr = 0;
    tss_b.iomap = 0x40000000;
    set_segmdesc(gdt + 1, 103, (int)&tss_a, AR_TSS32);
    set_segmdesc(gdt + 2, 103, (int)&tss_b, AR_TSS32);
    load_tr_desc(1 * 8);
    int task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024;
